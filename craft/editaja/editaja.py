...
class Package(CraftPackageObject.get("kde").pattern):
    ...
    def configure(self):
        # Craft has already fetched the pinned source and applied Phase 5, build fixes, Phase 6, Phase 12, Phase 13, Phase 14, Phase 15, the keyframe API compatibility fix, the native save-copy fix, the primary AI Agent sidebar fix, the Filmora-style Creator Workspace polish, the compact AI Agent toolbox, the one-time creator layout migration, the one-click layout restore, the AI request lifecycle guard and the asynchronous Film Context path, nonblocking indexer startup, asynchronous native silence/transcription analysis and bounded AI request timeout, its user-facing setting, bounded sidebar display, the Gemini-only 100-key pool, the final Gemini-only UI copy cleanup and Full Editor Control v1 here.
