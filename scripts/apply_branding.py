#!/usr/bin/env python3
"""Apply Update P5 Edit Aja branding on top of the Phase 5 Kdenlive-based source tree.
...
branding, window/about text and Windows icons are changed to Update P5 Edit Aja.
"""
...
APP_NAME = "Update P5 Edit Aja"
REPO_URL = "https://github.com/tonitarung099-creator/Update-P5-Edit-Aja"
...
def apply(root: Path, asset_dir: Path) -> None:
    ...
    insertion = marker + '    aboutData.addCredit(i18n("Update P5 Edit Aja contributors"), i18n("AI-agent integration, familiar workspace, fork branding"));\n'
    ...
    for rel in ("src/dialogs/Splash.qml", "src/dialogs/Simplesplash.qml"):
        path = root / rel
        text = read_text(path)
        text = text.replace('KI18n.i18n("Kdenlive")', 'KI18n.i18n("Update P5 Edit Aja")')
        text = text.replace("Kdenlive crashed on last start.", "Update P5 Edit Aja crashed on last start.")
        text = text.replace("Kdenlive was upgraded.", "Update P5 Edit Aja was upgraded.")
        write_text(path, text)

    # AI panel: display Update P5 Edit Aja while retaining kdenlive_* tool identifiers for
    # backwards compatibility with Phase 5 MCP/API clients.
    ai = root / "src/aiassistant/aiassistantwidget.cpp"
    text = read_text(ai)
    text = text.replace("control Kdenlive through MCP", "control Update P5 Edit Aja through MCP")
    text = text.replace("all Kdenlive actions", "all Update P5 Edit Aja actions")
    text = text.replace("exposed by Kdenlive", "exposed by Update P5 Edit Aja")
    text = text.replace("List Kdenlive actions available to the agent", "List Update P5 Edit Aja actions available to the agent")
    text = text.replace("Trigger any registered Kdenlive QAction", "Trigger any registered Update P5 Edit Aja QAction")
    text = text.replace("Open a Kdenlive dock panel", "Open an Update P5 Edit Aja dock panel")
    write_text(ai, text)

    agent = root / "src/aiassistant/openaicompatibleagent.cpp"
    text = read_text(agent)
    text = text.replace("AI video-editing agent embedded in Kdenlive", "AI video-editing agent embedded in Update P5 Edit Aja")
    text = text.replace("Timeline frame exported by Kdenlive", "Timeline frame exported by Update P5 Edit Aja")
    ...
    text = re.sub(r"^Name(?:\[[^\]]+\])?=.*$", lambda m: m.group(0).split("=", 1)[0] + "=Update P5 Edit Aja", text, flags=re.MULTILINE)
    ...
    text = re.sub(r"<name(?:\s+xml:lang=\"[^\"]+\")?>.*?</name>", lambda m: re.sub(r">.*?</", ">Update P5 Edit Aja</", m.group(0)), text)
    text = text.replace("<developer_name translate=\"no\">KDE</developer_name>", "<developer_name translate=\"no\">Update P5 Edit Aja contributors</developer_name>")
    text = text.replace(
        ...
        "<p>Update P5 Edit Aja is an open-source AI-assisted video editor based on Kdenlive. It keeps Kdenlive's mature editing engine while adding a familiar workspace and provider-neutral AI agent access through API, REST and MCP.</p>",
    )
